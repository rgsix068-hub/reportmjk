...
# Note: We use bookworm (Debian 12 stable) because Trixie/testing
# requires additional repo config for ttf-mscorefonts-installer
RUN echo "ttf-mscorefonts-installer msttcorefonts/accepted-mscorefonts-eula select true" | debconf-set-selections \
    && sed -i 's/ main/ main non-free/g' /etc/apt/sources.list.d/debian.sources 2>/dev/null \
