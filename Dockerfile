...
# ── System dependencies ─────────────────────────────────────
# Install fonts compatible with Arial (Liberation Sans) and
# Times New Roman (Liberation Serif). Available in main repo.
# app.py auto-detects Liberation Sans on Linux as fallback.
RUN apt-get update \
...
# Using Streamlit defaults for CORS to avoid config conflicts.
