...
def _try_libreoffice_pdf(data, photos):
    """Convert DOCX to PDF using LibreOffice headless (cross-platform).
    Memberikan hasil yang hampir identik dengan template DOCX asli."""
    try:
        import os
        import tempfile

        with tempfile.TemporaryDirectory() as tmpdir:
            tmp_docx = os.path.join(tmpdir, "report.docx")
            tmp_pdf = os.path.join(tmpdir, "report.pdf")

            # Generate DOCX via build_report (sama seperti win32com)
            docx_bytes = build_report(data, photos).getvalue()
            with open(tmp_docx, 'wb') as f:
                f.write(docx_bytes)

            # Gunakan xvfb-run untuk virtual display agar tabel & layout render sempurna
            # Set HOME untuk writable config directory LibreOffice
...
def build_report_pdf(data, photos):
    ...
    # Langkah 2: Coba win32com (Windows - hasil sempurna)
    win32_result = _try_win32com_pdf(data, photos)
    if win32_result is not None:
        return win32_result

    # Langkah 3: Coba LibreOffice (Linux/macOS - hasil hampir identik DOCX)
    libre_result = _try_libreoffice_pdf(data, photos)
    if libre_result is not None:
        return libre_result

    # Langkah 4: Fallback ke fpdf2 (cross-platform)
